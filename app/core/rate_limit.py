...
class RateLimiter:
    ...
    async def is_rate_limited(self, identifier: str, limit: int, window: int) -> Optional[int]:
        """
    ...
    async def cleanup(self):
        ...
        async with self._lock:
            ...
            for identifier in list(self._requests.keys()):
                ...
                self._requests[identifier] = [t for t in timestamps if now - t <= window]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        if not settings.RATE_LIMIT_ENABLED or any(request.url.path.startswith(p) for p in self.EXCLUDED_PATHS):
        ...
        if retry_after is not None:
            await log_event(
                "rate_limiter", "WARNING", "Rate limit exceeded",
