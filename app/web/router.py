...
async def list_unsubscribed(
    ...
    items = crud.get_unsubscribed_emails(
        db=db, limit=ITEMS_PER_PAGE, offset=offset, search=search, unsub_method=unsub_method
    )
    total_count = crud.count_unsubscribed_emails(
        db=db, search=search, unsub_method=unsub_method
    )
    total_pages = math.ceil(total_count / ITEMS_PER_PAGE) if total_count > 0 else 1

    current_filters = {"search": search, "unsub_method": unsub_method}
    
    # Override the href in the pagination template by passing it in the context
    def pagination_url(page_num: int) -> str:
        return f"?{build_query_params(current_filters, {'page': page_num})}"
    ...
    context = {
        ...
        "pagination_url": pagination_url, # Pass helper to template
        "export_url_csv": f"/web/export?{build_query_params(current_filters, {'format': 'csv'})}",
        "export_url_json": f"/web/export?{build_query_params(current_filters, {'format': 'json'})}",
    }
