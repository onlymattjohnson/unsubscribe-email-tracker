...
def test_transaction_rollback_on_failure(
    test_client: TestClient, db_session: Session, mocker
):
    """
    ...
    response = test_client.post(
        "/api/v1/unsubscribed_emails/", headers=AUTH_HEADERS, json=payload
    )
...
async def test_successful_api_action_is_logged(test_client: TestClient, mocker):
    ...
    mock_logger = mocker.patch(
        "app.api.v1.endpoints.unsubscribed_emails.log_event", new_callable=AsyncMock
    )
...
async def test_logging_system_fallback_integration(mocker):
    ...
    # Mock Discord alert
    mock_discord_alert = mocker.patch(
    ...
    result = await log_event(
        "test_app", "CRITICAL", "This is a test of the fallback system."
    )
    ...
    assert (
        "CRITICAL | test_app | This is a test of the fallback system."
        in discord_call_args[0]
    )
...
def test_export_with_large_dataset(test_client: TestClient, db_session: Session):
    ...
    records_to_create = [
        UnsubscribedEmail(
            sender_name=f"Bulk Sender {i}",
            sender_email=f"bulk{i}@test.com",
            unsub_method="direct_link",
        )
        for i in range(150)
    ]
    ...
    response = test_client.get(
        "/api/v1/unsubscribed_emails/export?format=csv", headers=AUTH_HEADERS
    )
