...
def test_filter_by_unsub_method(test_client: TestClient, diverse_db):
    response = test_client.get(API_URL, headers=AUTH_HEADERS, params={"unsub_method": "isp_level"})
...
def test_search(test_client: TestClient, diverse_db):
    ...
    response = test_client.get(API_URL, headers=AUTH_HEADERS, params={"search": "daily"})
    assert response.status_code == 200
...
def test_filter_by_date_from(test_client: TestClient, diverse_db):
    ...
    response = test_client.get(API_URL, headers=AUTH_HEADERS, params={"date_from": date_str})
    assert response.status_code == 200
...
def test_combine_filters(test_client: TestClient, diverse_db):
    ...
    response = test_client.get(API_URL, headers=AUTH_HEADERS, params={"search": "tech", "unsub_method": "direct_link"})
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 2
