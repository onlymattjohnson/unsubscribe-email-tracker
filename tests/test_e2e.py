...
def e2e_data(db_session: Session):
    ...
    records = [
        UnsubscribedEmail(
            sender_name="E2E Company A",
            sender_email="contact@e2e-a.com",
            unsub_method="direct_link",
        ),
        UnsubscribedEmail(
            sender_name="E2E Company B",
            sender_email="support@e2e-b.com",
...
def test_full_workflow(test_client: TestClient, e2e_data):
    ...
    list_response = test_client.get(
        "/api/v1/unsubscribed_emails/", headers=AUTH_HEADERS
    )
    ...
    filter_response = test_client.get(
        "/api/v1/unsubscribed_emails/", headers=AUTH_HEADERS, params=filter_params
    )
    ...
    export_response_csv = test_client.get(
        "/api/v1/unsubscribed_emails/export",
        headers=AUTH_HEADERS,
        params=export_params_csv,
    )
    ...
    export_response_json = test_client.get(
        "/api/v1/unsubscribed_emails/export",
        headers=AUTH_HEADERS,
        params=export_params_json,
    )
